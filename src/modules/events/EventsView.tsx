  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-150">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-900 flex items-center gap-2">
            ...
            {t.eventsView?.title || (language === 'it' ? 'Eventi & Calendario Temporale' : 'Events & Temporal Calendar')}
          </h1>
          <p className="text-xs text-slate-500 mt-0.5">
            {language === 'it'
              ? 'Modulo events • Cronologia, partecipanti, luoghi e pianificazione temporale.'
              : 'Module events • Temporal timeline, participants, places, and scheduling.'}
          </p>
        </div>
        ...
        <button
          ...
          className="px-4 py-2 rounded-xl bg-purple-600 hover:bg-purple-700 text-white text-xs font-semibold flex items-center gap-2 shadow-sm shadow-purple-500/20 transition-all self-start sm:self-auto"
        >
          <Plus className="w-4 h-4" />{' '}
          {t.eventsView?.addEvent || (language === 'it' ? 'Nuovo Evento' : 'New Event')}
        </button>
      </div>
      ...
      <div className="p-3 bg-white border border-slate-200 rounded-xl flex flex-wrap items-center justify-between gap-3 text-xs shadow-2xs">
        <div className="flex items-center gap-2 flex-1 min-w-[200px] max-w-md">
          ...
          <input
            ...
            placeholder={
              language === 'it'
                ? 'Cerca per titolo, descrizione o partecipanti...'
                : 'Search by title, description or participants...'
            }
            value={searchQ}
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <option value="planned">{language === 'it' ? 'Pianificato' : 'Planned'}</option>
            <option value="in_progress">{language === 'it' ? 'In corso' : 'In Progress'}</option>
            <option value="completed">{language === 'it' ? 'Completato' : 'Completed'}</option>
            <option value="cancelled">{language === 'it' ? 'Annullato' : 'Cancelled'}</option>
          </select>
        </div>
      </div>
      ...
      {filteredEvents.length === 0 ? (
        <div className="p-12 text-center bg-white border border-slate-200 rounded-2xl space-y-3">
          ...
          <div className="text-sm font-semibold text-slate-700">
            {language === 'it' ? 'Nessun evento trovato' : 'No events found'}
          </div>
          <p className="text-xs text-slate-500 max-w-sm mx-auto">
            {language === 'it'
              ? 'Nessun evento programmato o corrispondente ai filtri impostati.'
              : 'No events scheduled or matching the active filters.'}
          </p>
          <button
            type="button"
            onClick={() => setIsCreating(true)}
            className="px-3.5 py-1.5 rounded-lg bg-purple-600 hover:bg-purple-700 text-white text-xs font-semibold inline-flex items-center gap-1.5"
          >
            <Plus className="w-3.5 h-3.5" />
            {language === 'it' ? 'Crea Evento' : 'New Event'}
          </button>
        </div>
      ) : (
            ...
            >
              <div className="space-y-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <div className="w-11 h-11 rounded-xl bg-purple-50 border border-purple-200 flex items-center justify-center text-purple-700 shadow-2xs shrink-0">
                      <CalendarIcon className="w-5 h-5" />
                    </div>
                    <div className="min-w-0">
                      <h3 className="font-semibold text-sm text-slate-900 group-hover:text-purple-600 transition-colors truncate">
                        {event.title}
                      </h3>
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        {event.is_all_day && (
                          <span className="text-[10px] text-slate-500 font-mono">
                            {language === 'it' ? 'Tutto il giorno' : 'All day'}
                          </span>
                        )}
                      </div>
                    </div>
                  ...
                  <button
                    ...
                    title={language === 'it' ? 'Elimina evento' : 'Delete event'}
                  >
              ...
              <div className="space-y-2 pt-2 border-t border-slate-100 text-xs text-slate-600">
                <div className="flex items-center gap-1.5 font-mono text-[11px] text-slate-500">
                  ...
                  <span>
                    {new Date(event.start_time).toLocaleString(language === 'it' ? 'it-IT' : 'en-US', {
                      month: 'short',
                      day: 'numeric',
                      hour: '2-digit',
                      minute: '2-digit',
      ...
      <ConfirmDialog
        ...
        confirmLabel={language === 'it' ? 'Elimina Evento' : 'Delete Event'}
          ...
          <div className="w-full max-w-xl bg-white border border-slate-200 rounded-2xl shadow-2xl overflow-hidden">
            <div className="p-4 sm:p-6 border-b border-slate-200 flex items-center justify-between bg-slate-50/50">
              <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2">
                <CalendarIcon className="w-5 h-5 text-purple-600" />
                {language === 'it' ? 'Nuovo Evento in Calendario' : 'New Event in Calendar'}
              </h2>
              <button
                type="button"
            ...
            <form onSubmit={handleCreate} className="p-4 sm:p-6 space-y-4 text-xs">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="text-slate-700 block mb-1 font-medium">
                    {language === 'it' ? 'Inizio Data/Ora *' : 'Start Time *'}
                  </label>
                  <input
                    type="datetime-local"
                    required
                ...
                <div>
                  <label className="text-slate-700 block mb-1 font-medium">
                    {language === 'it' ? 'Fine Data/Ora' : 'End Time'}
                  </label>
                  <input
                    type="datetime-local"
                ...
                <div>
                  <label className="text-slate-700 block mb-1 font-medium">
                    {language === 'it' ? 'Luogo Collegato' : 'Linked Place'}
                  </label>
                  <select
                    ...
                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 outline-none"
                  >
                    <option value="">{language === 'it' ? '-- Nessun luogo --' : '-- No linked place --'}</option>
                    {places.map((p) => (
              ...
              <div>
                <label className="text-slate-700 block mb-1 font-medium">
                  {language === 'it' ? 'Descrizione & Dettagli' : 'Description & Details'}
                </label>
                <textarea
                  rows={2}
              ...
              <div className="flex justify-end gap-2 pt-4 border-t border-slate-200 bg-slate-50 -mx-4 -mb-4 sm:-mx-6 sm:-mb-6 p-4 sm:p-6">
                ...
                >
                  {language === 'it' ? 'Salva Evento' : 'Save Event'}
                </button>
              </div>
            </form>
          </div>
